...
# AGENTS.md — Ensemble Project Conventions

## Project Overview

Ensemble is a desktop application for orchestrating autonomous coding agents at scale. It uses Phoenix (Elixir) for the backend and UI, Tauri v2 (Rust) for the native desktop shell, and SQLite for persistence.

## Architecture

```
Tauri (Rust) → manages → Phoenix (Elixir) → renders → LiveView UI in WebView
                                           → manages → Agent subprocesses via Port
```

- **Phoenix** handles all business logic, real-time UI, and agent orchestration
- **Tauri** handles native window, system tray, notifications, auto-updates
- **SQLite** stores run history, configuration, and event log
- **PubSub** provides real-time event bus between components

## Code Quality Requirements

Every change must pass before merge:

1. `mix compile --warnings-as-errors` — zero compiler warnings
2. `mix credo --strict` — zero Credo issues
3. `mix assay` — zero Dialyzer warnings (typespecs on all public functions)
4. `mix test` — all tests pass

## Ensemble-Specific Conventions

- **Typespecs:** Every public function must have an `@spec`
- **Behaviours:** Use `@callback` for extensible interfaces (e.g., `AgentRunner`)
- **Contexts:** Business logic in `lib/ensemble/` contexts, web layer in `lib/ensemble_web/`
- **GenServers:** Each agent session is a GenServer managing a Port subprocess
- **PubSub topics:** Use `"run:<run_id>"` for per-run events, `"runs"` for collection-level events, `"events"` and `"events:<type>"` for the event bus
- **Error handling:** Return `{:ok, result}` or `{:error, reason}` tuples; never raise in business logic
- **Git:** Conventional commits (`feat:`, `fix:`, `refactor:`, `test:`, `docs:`), one logical change per commit
